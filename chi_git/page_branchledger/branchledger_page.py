...
class ChiGitBranchLedgerPage:
    def __init__(self, parent=None, app=None, page_key="", page_folder="", *args, **kwargs):
        ...
        self._style_prefix = f"ChiGitBranchLedger.{id(self)}"
    ...
    def _build_ui(self, parent):
        ...
        button = ttk.Button(repo_box, text="Refresh", command=self.refresh_all, style=f"{self._style_prefix}.TButton")
        button.grid(row=0, column=3, padx=6, pady=6)
        self._themed_buttons.append(button)
        button = ttk.Button(repo_box, text="Fetch", command=self.fetch_all, style=f"{self._style_prefix}.TButton")
        button.grid(row=0, column=4, padx=6, pady=6)
        self._themed_buttons.append(button)

        meta = ttk.LabelFrame(parent, text="Branch state", style=f"{self._style_prefix}.TLabelframe")
        meta.grid(row=1, column=0, sticky="ew", padx=8, pady=4)
        for i in range(4):
            meta.columnconfigure(i, weight=1)
        self._themed_labelframes.append(meta)
        ...
        toolbar = ttk.LabelFrame(parent, text="Actions", style=f"{self._style_prefix}.TLabelframe")
        toolbar.grid(row=2, column=0, sticky="ew", padx=8, pady=4)
        for i in range(7):
            toolbar.columnconfigure(i, weight=1)
        self._themed_labelframes.append(toolbar)
        for col, (text, cmd) in enumerate((
            ("Checkout local", self.checkout_selected_local),
            ("Track remote", self.checkout_selected_remote),
            ("Copy branch", self.copy_selected_branch),
            ("Copy commit", self.copy_selected_commit),
            ("Show changed files", self.show_selected_commit_files),
            ("Show commit details", self.show_selected_commit_details),
            ("Copy summary", self.copy_summary),
        )):
            button = ttk.Button(toolbar, text=text, command=cmd, style=f"{self._style_prefix}.TButton")
        ...
        local_box = ttk.Labelframe(middle, text="Local branches", style=f"{self._style_prefix}.TLabelframe")
        local_box.columnconfigure(0, weight=1)
        local_box.rowconfigure(1, weight=1)
        self._themed_labelframes.append(local_box)
        local_filter = ttk.Frame(local_box, style=f"{self._style_prefix}.Panel.TFrame")
        local_filter.grid(row=0, column=0, columnspan=2, sticky="ew", padx=6, pady=(6, 2))
        local_filter.columnconfigure(1, weight=1)
        self._themed_panel_frames.append(local_filter)
        label = ttk.Label(local_filter, text="Filter:", style=f"{self._style_prefix}.Panel.Muted.TLabel")
        label.grid(row=0, column=0, padx=(0, 4), pady=2)
        self._themed_panel_muted_labels.append(label)
        branch_entry = ttk.Entry(local_filter, textvariable=self.branch_filter_var, style=f"{self._style_prefix}.TEntry")
        ...
        self._themed_entries.append(branch_entry)
        ...
        remote_box = ttk.Labelframe(middle, text="Remote branches", style=f"{self._style_prefix}.TLabelframe")
        remote_box.columnconfigure(0, weight=1)
        remote_box.rowconfigure(1, weight=1)
        self._themed_labelframes.append(remote_box)
        remote_filter = ttk.Frame(remote_box, style=f"{self._style_prefix}.Panel.TFrame")
        remote_filter.grid(row=0, column=0, columnspan=2, sticky="ew", padx=6, pady=(6, 2))
        self._themed_panel_frames.append(remote_filter)
        label = ttk.Label(remote_filter, text="Uses same filter", style=f"{self._style_prefix}.Panel.Muted.TLabel")
        label.grid(row=0, column=0, padx=4, pady=2, sticky="w")
        self._themed_panel_muted_labels.append(label)
        ...
        commits_box = ttk.LabelFrame(parent, text="Recent commits", style=f"{self._style_prefix}.TLabelframe")
        ...
        self._themed_labelframes.append(commits_box)
        commit_filter = ttk.Frame(commits_box, style=f"{self._style_prefix}.Panel.TFrame")
        commit_filter.grid(row=0, column=0, columnspan=2, sticky="ew", padx=6, pady=(6, 2))
        commit_filter.columnconfigure(1, weight=1)
        self._themed_panel_frames.append(commit_filter)
        label = ttk.Label(commit_filter, text="Filter:", style=f"{self._style_prefix}.Panel.Muted.TLabel")
        label.grid(row=0, column=0, padx=(0, 4), pady=2)
        self._themed_panel_muted_labels.append(label)
        commit_entry = ttk.Entry(commit_filter, textvariable=self.commit_filter_var, style=f"{self._style_prefix}.TEntry")
        ...
        self._themed_entries.append(commit_entry)
        ...
        details_box = ttk.LabelFrame(parent, text="Commit details / files", style=f"{self._style_prefix}.TLabelframe")
        ...
        self._themed_labelframes.append(details_box)
    ...
    def _apply_theme(self):
        ...
        for widget in self._themed_buttons:
            try:
                ...
                pass
        apply_listbox_theme(self.local_list, self._theme_tokens)
        apply_listbox_theme(self.remote_list, self._theme_tokens)
        apply_listbox_theme(self.commit_list, self._theme_tokens)
        apply_text_theme(self.details_text, self._theme_tokens)
    ...
    def _apply_branch_filters(self):
        ...
        if not self.local_branch_rows and not self.remote_branch_rows:
            self._write_details("No branch data yet.\n\nChoose a valid repo and refresh to load branch information.")
        elif needle and not self.visible_local_rows and not self.visible_remote_rows:
            self._write_details(f"No branches match filter: {self.branch_filter_var.get().strip()}")
    ...
    def _apply_commit_filter(self):
        ...
        if not self.commit_rows:
            self._write_details("No commits loaded yet.\n\nSelect a repo and a branch to populate recent commits.")
        elif needle and not self.visible_commit_rows:
            self._write_details(f"No commits match filter: {self.commit_filter_var.get().strip()}")
    ...
    def show_selected_commit_details(self):
        ...
        self._write_commit_details(oid, res["text"])
    ...
    def show_selected_commit_files(self):
        ...
        self._write_commit_details(oid, res["text"], mode_label="Changed files")
    ...
    def _write_commit_details(self, oid, raw_text, mode_label="Commit details"):
        selected = next((row for row in self.visible_commit_rows if row.get("oid") == oid), None)
        summary_lines = [
            mode_label,
            "",
            f"Commit: {oid}",
        ...
        if selected:
            summary_lines.extend([
                f"Short id: {selected.get('short_oid', '') or '(unknown)'}",
                f"Date: {selected.get('date', '') or '(unknown)'}",
                f"Author: {selected.get('author', '') or '(unknown)'}",
                f"Subject: {selected.get('subject', '') or '(no subject)'}",
            ])
        summary_lines.extend([
            "",
            "Raw output",
            (raw_text or "").strip() or "(no output)",
        ])
        self._write_details("\n".join(summary_lines))
