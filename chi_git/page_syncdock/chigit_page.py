...
class ChiGitSyncDockPage:
    def __init__(self, parent=None, app=None, page_key="", page_folder="", *args, **kwargs):
        ...
        self._style_prefix = f"ChiGitSyncDock.{id(self)}"
    ...
    def _build_ui(self, parent):
        ...
        top = ttk.LabelFrame(parent, text="Repo", style=f"{self._style_prefix}.TLabelframe")
        top.grid(row=0, column=0, sticky="nsew", padx=8, pady=(8, 4))
        top.columnconfigure(1, weight=1)
        self._themed_labelframes.append(top)

        label = ttk.Label(top, text="Repo root:", style=f"{self._style_prefix}.Panel.TLabel")
        label.grid(row=0, column=0, sticky="w", padx=6, pady=6)
        self._themed_panel_labels.append(label)
        entry = ttk.Entry(top, textvariable=self.repo_var, style=f"{self._style_prefix}.TEntry")
        entry.grid(row=0, column=1, sticky="ew", padx=6, pady=6)
        self._themed_entries.append(entry)
        for col, (text, cmd) in enumerate((
            ("Browse", self.choose_repo),
            ("Open repo", self.open_repo_folder),
            ("Refresh", self.refresh_status),
        ), start=2):
            button = ttk.Button(top, text=text, command=cmd, style=f"{self._style_prefix}.TButton")
            button.grid(row=0, column=col, padx=6, pady=6)
            self._themed_buttons.append(button)

        meta = ttk.Frame(top, style=f"{self._style_prefix}.Panel.TFrame")
        ...
        branch_box = ttk.LabelFrame(parent, text="Branch tools", style=f"{self._style_prefix}.TLabelframe")
        branch_box.grid(row=1, column=0, sticky="ew", padx=8, pady=4)
        self._themed_labelframes.append(branch_box)
        for i in range(7):
            branch_box.columnconfigure(i, weight=1)

        label = ttk.Label(branch_box, text="Branch:", style=f"{self._style_prefix}.Panel.TLabel")
        label.grid(row=0, column=0, sticky="w", padx=6, pady=6)
        self._themed_panel_labels.append(label)
        entry = ttk.Entry(branch_box, textvariable=self.branch_entry_var, style=f"{self._style_prefix}.TEntry")
        entry.grid(row=0, column=1, columnspan=2, sticky="ew", padx=6, pady=6)
        self._themed_entries.append(entry)
        for col, (text, cmd) in enumerate((
            ("Fill current", self.fill_current_branch),
            ("Checkout", self.checkout_branch),
            ("Create branch", self.create_branch_from_current),
            ("Copy status", self.copy_status_summary),
        ), start=3):
            button = ttk.Button(branch_box, text=text, command=cmd, style=f"{self._style_prefix}.TButton")
            button.grid(row=0, column=col, sticky="ew", padx=4, pady=6)
            self._themed_buttons.append(button)

        actions = ttk.LabelFrame(parent, text="Sync actions", style=f"{self._style_prefix}.TLabelframe")
        ...
        for col, (text, cmd) in enumerate((
            ...
            ("Refresh log", self._refresh_log_view),
            ("Commit snapshot", self.commit_changes),
        )):
            button = ttk.Button(actions, text=text, command=cmd, style=f"{self._style_prefix}.TButton")
            button.grid(row=0, column=col, sticky="ew", padx=4, pady=6)
            self._themed_buttons.append(button)

        files = ttk.LabelFrame(parent, text="Changed files", style=f"{self._style_prefix}.TLabelframe")
        ...
        commit = ttk.LabelFrame(parent, text="Commit + work note", style=f"{self._style_prefix}.TLabelframe")
        commit.grid(row=4, column=0, sticky="ew", padx=8, pady=4)
        self._themed_labelframes.append(commit)
        commit.columnconfigure(1, weight=1)

        label = ttk.Label(commit, text="Commit message:", style=f"{self._style_prefix}.Panel.TLabel")
        label.grid(row=0, column=0, sticky="w", padx=6, pady=6)
        self._themed_panel_labels.append(label)
        entry = ttk.Entry(commit, textvariable=self.commit_msg_var, style=f"{self._style_prefix}.TEntry")
        entry.grid(row=0, column=1, sticky="ew", padx=6, pady=6)
        self._themed_entries.append(entry)
        label = ttk.Label(commit, text="Work note:", style=f"{self._style_prefix}.Panel.TLabel")
        label.grid(row=1, column=0, sticky="w", padx=6, pady=6)
        self._themed_panel_labels.append(label)
        entry = ttk.Entry(commit, textvariable=self.work_note_var, style=f"{self._style_prefix}.TEntry")
        entry.grid(row=1, column=1, sticky="ew", padx=6, pady=6)
        self._themed_entries.append(entry)
        button = ttk.Button(commit, text="Save note only", command=self.save_note_only, style=f"{self._style_prefix}.TButton")
        button.grid(row=0, column=2, rowspan=2, sticky="ns", padx=6, pady=6)
        self._themed_buttons.append(button)

        output = ttk.LabelFrame(parent, text="Command output", style=f"{self._style_prefix}.TLabelframe")
        ...
        output_toolbar = ttk.Frame(output, style=f"{self._style_prefix}.Panel.TFrame")
        output_toolbar.grid(row=1, column=0, columnspan=2, sticky="ew", padx=6, pady=(0, 6))
        self._themed_panel_frames.append(output_toolbar)
        button = ttk.Button(output_toolbar, text="Clear output", command=self.clear_output, style=f"{self._style_prefix}.TButton")
        ...
        history = ttk.LabelFrame(parent, text="Recent sync log", style=f"{self._style_prefix}.TLabelframe")
        ...
        button.pack(side="left", padx=(0, 4))
        self._themed_buttons.append(button)
        button = ttk.Button(log_toolbar, text="Open log folder", command=self.open_data_folder, style=f"{self._style_prefix}.TButton")
        button.pack(side="left", padx=4)
        self._themed_buttons.append(button)
        ...
        label = ttk.Label(status, textvariable=self.refresh_note_var, style=f"{self._style_prefix}.Muted.TLabel")
