...
GUICHI_FILES = os.path.join(SHELL_DIR, "gui_files")
...
SIDEWINDOW_REGISTRY_PATH = os.path.join(STATE_DIR, "sidewindow_registry.json")

# Ensure gui_files is importable
...
def load_config():
    ...
    defaults = {
        ...
        "known_roots": [],
        "font_scale": 1.0,
        "page_theme_global": None,
        "page_theme_pack_overrides": {},
        "page_theme_page_overrides": {},
        "shortcut_overrides": {},
        "interaction_wheel_scroll_enabled": True,
        "interaction_ctrl_a_enabled": True,
        "interaction_ctrl_c_enabled": True,
        "interaction_escape_dismiss_enabled": True,
    }
...
def action_discover_sidewindows(config, sw_registry, root=None):
    """
    Scan root for chiside_* folders. Merge into sw_registry. Save. Update config.
    root defaults to config["last_selected_root"] or SHELL_DIR.
    Returns (result_dict, merge_actions).
    """
    import sidewindow_discovery
    import sidewindow_registry

    if root is None:
        root = config.get("last_selected_root") or SHELL_DIR

    result = sidewindow_discovery.discover(root)

    if result["scan_errors"]:
        return result, []

    merge_actions = sidewindow_registry.merge_findings(sw_registry, result["findings"])
    sidewindow_registry.save_registry(sw_registry, SIDEWINDOW_REGISTRY_PATH)
