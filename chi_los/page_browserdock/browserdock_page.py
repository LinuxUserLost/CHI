...
def _bind_scroll(widget):
    """Use the Guichi universal wheel helper."""
...
class PageBrowserdock:
    ...
    def __init__(self, parent, app=None, page_key="", page_folder="",
        ...
        self._style_prefix = f"BrowserBookmarks.{id(self)}"
    ...
    def _build_list_pane(self, parent):
        left = ttk.LabelFrame(parent, text="Saved Bookmarks", padding=(4, 2))
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 4))
        left.columnconfigure(0, weight=1)
        left.rowconfigure(0, weight=1)
        self._left_outer = left
    ...
    def _build_editor_pane(self, parent):
        right = ttk.LabelFrame(parent, text="Bookmark Editor", padding=(6, 4))
        right.grid(row=0, column=1, sticky="nsew")
        right.columnconfigure(1, weight=1)
        self._right_outer = right
    ...
    def _apply_theme(self):
        ...
        if getattr(self, "_txt_notes", None) is not None:
            try:
                self._txt_notes.configure(
                    background=list_bg,
                    foreground=list_fg,
    ...
    def _load_tools(self):
        ...
        if not os.path.isfile(self.tools_path):
            self._tools = []
            self._refresh_list()
            self._set_status("No bookmarks file yet \u2014 add your first entry.")
            return
        try:
        ...
        self._tools = cleaned
        self._refresh_list()
        self._set_status(f"Loaded {len(self._tools)} bookmark(s).")
    ...
    def _save_tools(self):
        ...
        try:
            ...
            self._set_status(f"Saved {len(self._tools)} bookmark(s).")
        except Exception as ex:
    ...
    def _duplicate_tool(self):
        if not (0 <= self._selected_idx < len(self._tools)):
            self._set_status("Select a bookmark to duplicate.")
            return
    ...
    def _delete_tool(self):
        if not (0 <= self._selected_idx < len(self._tools)):
            self._set_status("Select a bookmark to delete.")
            return
