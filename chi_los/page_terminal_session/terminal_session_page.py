...
def _find_pack_root() -> str:
    """
    Return the nearest chipack root for this page.
    A pack root is identified by containing both module_manifest.json and pages.json.
    """
    here = Path(__file__).resolve()
    for candidate in [here.parent, *here.parents]:
...
class SaveCommandPopup(tk.Toplevel):
    ...
    def __init__(self, parent, linuxcommands_dir, prefill_cmd, on_save_callback, metadata=None):
    ...
    def _on_save(self):
        ...
        record    = {
            ...
            "command":    cmd_text,
            "notes":      "",
            "source_task": self._metadata.get("source_task", ""),
            "source_subtask": self._metadata.get("source_subtask", ""),
            "pack_root": self._metadata.get("pack_root", ""),
            "working_dir_hint": self._metadata.get("working_dir_hint", ""),
            "verify_command": self._metadata.get("verify_command", ""),
            "requires_sudo": bool(self._metadata.get("requires_sudo", False)),
            "risk_level": self._metadata.get("risk_level", "unknown"),
            "host_scope": self._metadata.get("host_scope", "manual_review"),
            "procedure_group": self._metadata.get("procedure_group", ""),
            "created_at": now,
            "updated_at": now,
        }
        try:
...
class PageTerminalSession:
    ...
    def __init__(self, parent, app=None, page_key="", page_folder="",
        ...
        self._style_prefix = f"TerminalSession.{id(self)}"
    ...
    def _build_top_bar(self):
        ...
        self._shell_status_label = ttk.Label(
            bar,
            textvariable=self._shell_status_var,
            foreground="#666",
            font=("", 8),
            anchor="w",
        )
        self._shell_status_label.grid(row=0, column=6, sticky="w", padx=(8, 4))
    ...
    def _build_notebook(self):
        ...
        self._build_session_tab(workspace)
    ...
    def _apply_theme(self):
        ...
        try:
            ...
            style.configure(
                f"{self._style_prefix}.TPanedwindow",
                background=tokens["content_bg"],
            )
            style.configure(
            ...
            style.configure(
                f"{self._style_prefix}.PanelMuted.TLabel",
                background=tokens["panel_bg"],
                foreground=tokens["text_muted"],
            )
            style.configure(
        ...
        text_bg = tokens["panel_bg"]
        muted = tokens["text_muted"]
        accent = tokens["accent"]

        for widget in (
            getattr(self, "_sc_lb", None),
            getattr(self, "_ce_lb", None),
            getattr(self, "_queue_lb", None),
        ):
            if widget is None:
                continue
            try:
        ...
        for widget in (
            getattr(self, "_session_view", None),
            getattr(self, "_cmd_input", None),
            getattr(self, "_ce_command", None),
            getattr(self, "_ce_notes", None),
            getattr(self, "_ce_mv_text", None),
        ):
            if widget is None:
                continue
            try:
        ...
        for widget in (
            getattr(self, "_ce_canvas", None),
        ):
            if widget is None:
                continue
            try:
                widget.configure(background=tokens["panel_bg"])
            except Exception:
                pass
        ...
        if getattr(self, "_session_view", None) is not None:
            try:
                self._session_view.tag_configure(
                    "cmd_selected", background=tokens["border"], foreground=tokens["text_active"]
                )
                self._session_view.tag_configure(
                    "cmd_header", foreground=accent
                )
                self._session_view.tag_configure(
                    "cmd_text", foreground="#5cc37a"
                )
                self._session_view.tag_configure(
                    "terminal_output", foreground=list_fg
                )
                self._session_view.tag_configure(
                    "shell_marker", foreground=muted
                )
            except Exception:
                pass

        if getattr(self, "_ce_mv_text", None) is not None:
            try:
                self._ce_mv_text.tag_configure(
                    "hl_field",
                    background=tokens["border"],
                    foreground=tokens["text_active"],
                    selectbackground=tokens["accent"],
                )
                self._ce_mv_text.tag_configure("json_key", foreground=accent)
                self._ce_mv_text.tag_configure("json_str", foreground="#7bcf8f")
                self._ce_mv_text.tag_configure("json_lit", foreground="#f3ad66")
            except Exception:
                pass

        for widget, style_name in (
            (getattr(self, "_shell_status_label", None), f"{self._style_prefix}.Muted.TLabel"),
            (getattr(self, "_root_label", None), f"{self._style_prefix}.Muted.TLabel"),
            (getattr(self, "_status_label", None), f"{self._style_prefix}.Muted.TLabel"),
        ):
            if widget is None:
                continue
            try:
        ...
        for child in widget.winfo_children():
            try:
                if isinstance(child, ttk.LabelFrame):
                    ...
                    child.configure(style=f"{self._style_prefix}.TPanedwindow")
    ...
    def _choose_root(self):
        d = filedialog.askdirectory(title="Select chi_los pack root")
        if d:
            self._set_root(d)
    ...
    def _build_session_tab(self, parent):
        ...
        # Row 2 (expand): saved commands | terminal | command details
        ...
        self._sc_lb.bind("<<ListboxSelect>>", self._sc_on_select)
        self._sc_lb.bind("<Double-1>", lambda _e: self._sc_add_command())
        ...
        # ── Right: command details / editor ─────────────────────────────────
        detail_outer = ttk.LabelFrame(paned, text="Command Details", padding=4)
        detail_outer.columnconfigure(0, weight=1)
        detail_outer.rowconfigure(0, weight=1)
        self._build_command_details_panel(detail_outer)
    ...
    def _build_command_details_panel(self, parent):
        ...
        inner.bind("<Map>", lambda _e: _rebind(inner))
        ...
        self._ce_info_var = tk.StringVar(value="Select a saved command to inspect or edit.")
        ...
        ttk.Button(save_row, text="New", width=8,
                   command=self._ce_new).pack(side="left", padx=2)
        ttk.Button(save_row, text="Save", width=8,
        ...
        ttk.Button(save_row, text="Delete", width=8,
    ...
    def _sc_on_select(self, event=None):
        sel = self._sc_lb.curselection()
        if not sel:
            return
        label = self._sc_lb.get(sel[0])
    ...
    def _save_session_log(self):
        ...
        for ci, chunk in enumerate(chunks):
            ...
            header_lines = [
                ...
                f"pack_root: {_yaml_scalar(self.pack_root)}",
                f"cwd_at_save: {_yaml_scalar(self._cwd_var.get())}",
                f"chunk_index: {ci + 1}",
    ...
    def _save_command_as(self):
        ...
        metadata = {
            "source_task": self._task_var.get().strip(),
            "source_subtask": self._subtask_var.get().strip(),
            "pack_root": self.pack_root,
            "working_dir_hint": self._cwd_var.get(),
            "verify_command": "",
            "requires_sudo": False,
            "risk_level": "unknown",
            "host_scope": "manual_review",
            "procedure_group": self._task_var.get().strip(),
        }
    ...
    def _ce_delete(self):
        path = self._ce_json_path
        if not path:
            if not hasattr(self, "_ce_lb"):
                messagebox.showinfo("Delete", "Open a saved command first."); return
            sel = self._ce_lb.curselection()
            if not sel:
