"""
page_qwen_tts / qwen_backend.py
────────────────────────────────────────────────────────────────────────────
Local Qwen TTS backend adapter for Guichi shell — pagepack_chillama.
(Thin re-export — page uses helpers/qwen_tts_backend.py as the canonical copy.)

Calls helpers/qwen_tts_infer.py via the qwen3tts-venv Python interpreter.
...
# Batch inference script — lives in helpers/ alongside qwen_tts_backend.py
_HELPERS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "..", "helpers")
SCRIPT_PATH  = os.path.normpath(
    os.path.join(_HELPERS_DIR, "qwen_tts_infer.py"))
...
class QwenBackend:
    ...
    def generate(self, text: str, model_id: str, voice_id: str) -> dict:
        """
        ...
        voice_id : voice identifier string (see CUSTOM_VOICES in qwen_tts_page.py)
        ...
        try:
            ...
            return {"ok": False,
                    "error": f"Python interpreter not found: {self.venv_python}"}
