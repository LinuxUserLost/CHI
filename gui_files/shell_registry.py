...
def save_registry(registry, registry_path):
    """Serialize registry to disk, auto-creating parent directories if needed."""
...
def clear_registry(registry):
    """Reset an in-memory registry to an empty valid state."""
    registry.clear()
    registry.update(_empty_registry())
...
def _pack_entry_from_finding(finding):
    ...
    return {
        ...
        "pack_name": finding.get("pack_name"),
        "module_title": finding.get("module_title"),
        "description": finding.get("description", ""),
        "home_description": finding.get("home_description", ""),
        "custom_nav_name": None,
        "display_suffix": "",
        "status": pack_status,
        "hidden": False,
        "pages": page_entries,
...
def _carry_custom_nav_names(existing_entry, new_entry):
    """Preserve custom nav names across discovery updates and rebuilds."""
    new_entry["custom_nav_name"] = existing_entry.get("custom_nav_name")

    existing_pages = {
        page.get("page_id"): page
        for page in existing_entry.get("pages", [])
        if page.get("page_id")
    }
    for page in new_entry.get("pages", []):
        existing_page = existing_pages.get(page.get("page_id"))
        if existing_page is not None:
            page["custom_nav_name"] = existing_page.get("custom_nav_name")
...
def merge_findings(registry, findings_list):
    ...
    for finding in findings_list:
        ...
        if existing_idx >= 0:
            ...
            existing_entry = registry["packs"][existing_idx]
            was_hidden = existing_entry.get("hidden", False)
            new_entry["hidden"] = was_hidden
            _carry_custom_nav_names(existing_entry, new_entry)
...
def rebuild_registry(registry, discover_fn, dev_mode=False):
    ...
    for entry in registry["packs"]:
        ...
        if finding is None:
            ...
            _carry_custom_nav_names(entry, refreshed)
...
def set_pack_custom_name(registry, pack_id, source_path, custom_name):
    """Set or clear a custom nav name for a pack."""
    idx = _find_pack_index(registry, pack_id, source_path)
    if idx < 0:
        return False
    registry["packs"][idx]["custom_nav_name"] = custom_name or None
    return True


def set_page_custom_name(registry, pack_id, source_path, page_id, custom_name):
    """Set or clear a custom nav name for a page."""
    idx = _find_pack_index(registry, pack_id, source_path)
    if idx < 0:
        return False
    for page in registry["packs"][idx].get("pages", []):
        if page.get("page_id") == page_id:
            page["custom_nav_name"] = custom_name or None
            return True
    return False
