...
# Probed GUI mount method names, in order — first match wins.
...
def probe_page_gui_method(page_instance, parent_frame):
    """
    Try each method in _PAGE_GUI_METHODS on page_instance; call the first found.

    Detects whether the method expects a parent argument via inspect.signature.
    Returns (success, method_name, error_detail):
      (True,  name, None)  — called successfully
      (False, name, str)   — method raised an exception
