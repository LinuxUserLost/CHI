...
# Canonical names for required discovery files and fields.
# All four constants are the single source of truth — code must reference
# them rather than repeating literal strings inline.
...
def _validate_manifest(manifest_data, folder_path, dev_mode=False, suppress_dev_warnings=False):
    ...
    for field in REQUIRED_PACK_FIELDS:
        ...
        if not val:
            if dev_mode and field == "pack_id":
                generated_id = f"dev_{uuid.uuid4().hex[:8]}"
                pack_info[field] = generated_id
                if not suppress_dev_warnings:
                    warnings.append(f"missing {field} — dev-mode generated: {generated_id}")
            else:
                errors.append(f"missing required field: {field}")
                pack_info[field] = None
        else:
            pack_info[field] = val

    pack_info["pack_name"] = manifest_data.get("pack_name")
    pack_info["module_title"] = manifest_data.get("module_title")
    description = manifest_data.get("description")
    if isinstance(description, str):
        pack_info["description"] = description
    home_description = manifest_data.get("home_description")
    if isinstance(home_description, str):
        pack_info["home_description"] = home_description
...
def _scan_folder(folder_path, dev_mode=False, suppress_dev_warnings=False):
    ...
    A folder is "valid" if it contains at least one of the files listed in
    REQUIRED_DISCOVERY_FILES (module_manifest.json, pages.json). Folders with
    neither file are silently skipped (return None). Folders missing only one
    file produce errors unless dev_mode is True.

    Returns a pack finding dict, or None if the folder is not a pack candidate.
    """
    manifest_path = os.path.join(folder_path, REQUIRED_DISCOVERY_FILES[0])
    pages_path = os.path.join(folder_path, REQUIRED_DISCOVERY_FILES[1])
    ...
    if not has_manifest:
        if dev_mode:
            if not suppress_dev_warnings:
                finding["warnings"].append(
                    f"{REQUIRED_DISCOVERY_FILES[0]} missing — dev-mode best-effort"
                )
        else:
            finding["errors"].append(f"missing required file: {REQUIRED_DISCOVERY_FILES[0]}")
    else:
        ...
        if read_err:
            ...
            finding["pack_name"] = pack_info.get("pack_name")
            finding["module_title"] = pack_info.get("module_title")
            finding["description"] = pack_info.get("description", "")
            finding["home_description"] = pack_info.get("home_description", "")
    ...
    if not has_pages:
        if dev_mode:
            if not suppress_dev_warnings:
                finding["warnings"].append(
                    f"{REQUIRED_DISCOVERY_FILES[1]} missing — dev-mode best-effort"
                )
        else:
            finding["errors"].append(f"missing required file: {REQUIRED_DISCOVERY_FILES[1]}")
    else:
