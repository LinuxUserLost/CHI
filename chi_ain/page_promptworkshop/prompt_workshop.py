...
Storage:
  /chi_ain/promptworkshop/prompts/   — prompt record JSON files
  /chi_ain/promptworkshop/maps/      — prompt map JSON files
...
def _make_listbox(parent, height=8):
    ...
    interaction_support.setup_listbox_widget(lb)
    return frm, lb
...
class PagePromptWorkshop:
    ...
    def __init__(self, parent: tk.Widget, app=None, page_key: str = "",
        ...
        self._style_prefix = f"PromptWorkshop.{id(self)}"
        self._style = None
    ...
    def _build_top_bar(self):
        ...
        self._path_label = ttk.Label(bar, textvariable=self._path_var, anchor="w")
        self._path_label.grid(row=0, column=99, sticky="ew", padx=4)
    ...
    def _build_status_bar(self):
        ...
        self._status_lbl = ttk.Label(bar, textvariable=self._status_var, anchor="w")
    ...
    def _auto_find_root(self):
        ...
        if root:
            self._set_root(root)
            self._set_status(f"Auto-found workshop root: {root}")
            return
        self._set_status("Workshop root not found \u2014 use Choose Root.")

    def _choose_root(self):
        initial_dir = self.workshop_root or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        d = filedialog.askdirectory(title="Select promptworkshop directory", initialdir=initial_dir)
        if d:
            self._set_root(d)

    def _set_root(self, path: str):
        root = promptworkshop_store.normalize_workshop_root(path)
        if not root:
            self._set_status("Invalid workshop root.")
            return
        self.workshop_root = root
        self.prompts_dir, self.maps_dir = promptworkshop_store.ensure_workshop_dirs(root)
        short = root if len(root) <= 72 else "\u2026" + root[-69:]
        self._path_var.set(f"Workshop Root: {short}")
    ...
    def _build_prompt_editor_tab(self, parent):
        ...
        interaction_support.setup_text_widget(self._pe_body)
        ...
        self._pe_info_lbl = ttk.Label(inner, textvariable=self._pe_info_var)
        self._pe_info_lbl.grid(row=row, column=0, columnspan=2, sticky="w", pady=(4, 0))
        row += 1
    ...
    def _pe_refresh_file_list(self):
        ...
        if not self.prompts_dir or not os.path.isdir(self.prompts_dir):
            self._pe_lb.insert("end", "(set workshop root to load prompts)")
            return
        records = promptworkshop_store.list_records(self.prompts_dir)
        if not records:
            self._pe_lb.insert("end", "(no saved prompts yet)")
            return
        for display, full in records:
    ...
    def _pe_load_file(self, path):
        try:
            self._pe_record = promptworkshop_store.load_json_record(path, _empty_prompt)
    ...
    def _pe_write_json(self, path):
        try:
            promptworkshop_store.write_json_record(path, self._pe_record)
        except Exception as exc:
    ...
    def _build_prompt_maps_tab(self, parent):
        ...
        interaction_support.setup_text_widget(self._pm_preview)
        ...
        self._pm_info_lbl = ttk.Label(inner, textvariable=self._pm_info_var)
        self._pm_info_lbl.grid(row=row, column=0, columnspan=2, sticky="w", pady=(4, 0))
        row += 1
    ...
    def _pm_refresh_file_list(self):
        ...
        if not self.maps_dir or not os.path.isdir(self.maps_dir):
            self._pm_lb.insert("end", "(set workshop root to load maps)")
            return
        records = promptworkshop_store.list_records(self.maps_dir)
        if not records:
            self._pm_lb.insert("end", "(no saved maps yet)")
            return
        for display, full in records:
    ...
    def _pm_load_file(self, path):
        try:
            self._pm_record = promptworkshop_store.load_json_record(path, _empty_map)
        except Exception as exc:
            messagebox.showerror("Load Error", f"Could not load:\n{path}\n\n{exc}"); return
    ...
    def _pm_add_prompt_block(self):
        if not self.prompts_dir or not os.path.isdir(self.prompts_dir):
            ...
            return
        available = promptworkshop_store.list_records(self.prompts_dir)
        if not available:
        ...
        for display, _full_path in available:
        ...
        def _ok():
            ...
            if not sel: return
            idx = sel[0]
            display, _full_path = available[idx]
            ref = display
    ...
    def _pm_refresh_preview(self):
        assembled = promptworkshop_store.assemble_map_preview(self.prompts_dir, self._pm_record)
    ...
    def _pm_write_json(self, path):
        try:
            promptworkshop_store.write_json_record(path, self._pm_record)
        except Exception as exc:
            messagebox.showerror("Save Error", f"Failed to write:\n{exc}"); return
        fname = os.path.basename(path)
    ...
    def _apply_theme(self):
        ...
        try:
            ...
            self._style.configure(f"{self._style_prefix}.TLabelframe", background=t["panel_bg"], bordercolor=t["border"])
            self._style.configure(f"{self._style_prefix}.TLabelframe.Label", background=t["panel_bg"], foreground=t["text_main"])
            ...
            self._style.configure(f"{self._style_prefix}.TButton", background=t["button_bg"], foreground=t["text_main"])
            self._style.map(
                f"{self._style_prefix}.TButton",
                background=[("active", t["button_hover"])],
                foreground=[("active", t["text_active"]), ("disabled", t["button_disabled"])],
            )
            self._style.configure(f"{self._style_prefix}.TEntry", fieldbackground=t["panel_bg"], foreground=t["text_main"])
            self._style.configure(f"{self._style_prefix}.TNotebook", background=t["content_bg"], borderwidth=0)
            self._style.configure(f"{self._style_prefix}.TNotebook.Tab", background=t["button_bg"], foreground=t["text_main"], padding=(10, 4))
            self._style.map(
                f"{self._style_prefix}.TNotebook.Tab",
                background=[("selected", t["panel_bg"]), ("active", t["button_hover"])],
                foreground=[("selected", t["text_active"])],
            )
            self._style.configure(f"{self._style_prefix}.TCheckbutton", background=t["panel_bg"], foreground=t["text_main"])
        except Exception:
            pass

        try:
            self.frame.configure(style=f"{self._style_prefix}.TFrame")
            self._nb.configure(style=f"{self._style_prefix}.TNotebook")
        except Exception:
            pass

        self._apply_ttk_theme_tree(self.frame)

        for widget in (getattr(self, "_pe_lb", None), getattr(self, "_pm_lb", None), getattr(self, "_pm_blk_lb", None)):
            if widget is None:
                continue
            try:
                widget.configure(
        ...
        for widget in (
            getattr(self, "_pe_body", None),
            getattr(self, "_pm_preview", None),
            getattr(self, "_pe_mv_text", None),
            getattr(self, "_pm_mv_text", None),
        ):
            if widget is None:
                continue
            try:
        ...
        for widget in (getattr(self, "_pe_canvas", None), getattr(self, "_pm_canvas", None)):
            if widget is None:
                continue
            try:
                widget.configure(background=t["content_bg"])
            except Exception:
                pass

        for label in (
            getattr(self, "_path_label", None),
            getattr(self, "_status_lbl", None),
            getattr(self, "_pe_info_lbl", None),
            getattr(self, "_pm_info_lbl", None),
        ):
            if label is None:
                continue
            try:
        ...
        for mv in (getattr(self, "_pe_mv_text", None), getattr(self, "_pm_mv_text", None)):
            if mv is None:
                continue
            try:
                mv.tag_configure("hl_field", background=t["divider"], foreground=t["text_active"], selectbackground=t["divider"])
                mv.tag_configure("json_key", foreground=t["accent"])
                mv.tag_configure("json_str", foreground="#a6e3a1")
                mv.tag_configure("json_lit", foreground="#fab387")
            except Exception:
                pass
